...
export default async function handler(req, res) {
  ...
  if (dayUTC !== 25) return res.status(400).json({ message: 'Payouts can only be requested on the 25th of each month (UTC).' });

  try {
    await supabaseAdmin.from('payout_requests').insert([{ email: v.payload.email, requested_at: now.toISOString() }]);
  } catch (e) {}
